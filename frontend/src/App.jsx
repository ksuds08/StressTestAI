  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-bg text-gray-200">
      ...
      <main className="flex-1 min-h-0 flex flex-col">
          ...
          {messages.map((m, idx) => (
            ...
            >
              ...
              {m.role === 'thinking' ? (
                ...
                <ReactMarkdown className="prose prose-invert text-sm leading-snug max-w-none whitespace-pre-line">
                  {m.content.replace(/\n{3,}/g, '\n\n')}
                </ReactMarkdown>
              )}
            </div>
        ...
        {activeId && (
          <div className="mx-auto w-full sm:w-[90%] md:w-[75%] lg:w-[60%] xl:w-[50%] p-4 border-t border-gray-800 flex items-center gap-2 bg-[#1a1a1a]">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && send()}
              className="flex-1 border-none rounded-md px-4 py-4 bg-[#2c2c2c] text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-accent text-base"
              placeholder="Type your request..."
            />
            <button
              onClick={send}
              disabled={loading}
              className="px-3 py-3 bg-accent text-white rounded-md font-medium hover:opacity-90 disabled:opacity-50 transition"
            >
              <FiSend size={20} />
            </button>
          </div>
